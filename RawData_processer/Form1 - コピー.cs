...
namespace RawData_processer
{
    public partial class Form1 : Form
    {
        ...
        private void Trim_Start_Click(object sender, EventArgs e)
        {
            ...
            progressBar1.Maximum = total_datanum;
            ...
            using (StreamReader sr = new StreamReader(rawdata_filename, System.Text.Encoding.Default))
            {
                ...
                while (sr.Peek() >= 0)
                {
                    ...
                    if (Row_count % Constants.DATA_SEGMENT == 0 && i > line_datastart)
                    {
                        ...
                        if (cropbytime_cBox.Checked)
                        {
                            for (j = 0; j < Constants.DATA_SEGMENT; j = j + 1)
                            {
                                if (buffer[time_col, j] > starttime && buffer[time_col, j] <= endtime)
                                {
                                    ...
                                    for (k = 0; k < col_num; k++)
                                    {
                                        if (k == 0) sTemp += buffer[k, j];
                                        else sTemp += "," + buffer[k, j];
                                    }
                                    sTemp += "\r\n";
                                    sw.Write(sTemp);
                                }
                        ...
                        stTemp = String.Empty;
                        for (j = 0; j < fSpan.Length; j++) { stTemp += String.Format(" {0:e}", mag[j]); }
                        sw.WriteLine(stTemp);

                        if (i > line_dataend - pindex)
                        {
                            sw.WriteLine("END");
                            sw.WriteLine("X Display;AppendImage stft2D");
                            sw.WriteLine(String.Format("X SetScale/I x,{0:e},{1:e}, stft2D", 0, fSpan[fSpan.Length - 1]));
                            sw.WriteLine(String.Format("X SetScale/I y,{0:e},{1:e}, stft2D", rawdata_starttime, rawdata_endtime));
                        }
